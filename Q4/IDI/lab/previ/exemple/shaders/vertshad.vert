...
{	
	...
	// Direccio llum
	vec3 L = normalize(posF - vertSCO);
	
	// Passar vector normal a SCO
	mat3 NormalMatrix = (inverse(transpose(mat3(view * TG))));
	vec3 NM = normalize(NormalMatrix * normal);
	
	// Nomes la Normal i la L son "vectors", els unics que es normalitzen
	// Si volem que posfocus sigui exactament on esta la càmera
    	// l'inicialitzem a zero i no multipliquem per la view.
...
//clean phong:
    ...
    vec3 NormSCO = normalize(normMat*normal);
    vec4 vertexSCO = view*TG*vec4(vertex,1.0);

    // Si volem que posfocus sigui exactament on esta la càmera
    //linicialitzem a zero i no multipliquem per la view.
