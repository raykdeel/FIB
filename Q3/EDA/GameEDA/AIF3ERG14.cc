  ...
  static Player* factory () {
    ...
    bool adjacent_weapon(Pos p) {
        for(int k = 0; k < 8; k++) {
            ...
            and (cell(ii,jj).weapon == Gun or cell(ii,jj).weapon == Bazooka)) return true;
        }
        return false;
    }
        ...
        if(citizen(id).type == Warrior) {
            if(is_round_night(round()) or (round() < num_rounds() and is_round_night(round()+5))){
                for(int k = 0; k < 4; k++) {
                    ...
                    if(pos_ok(Pos(ii,jj)) and cell(ii,jj).id != -1 and citizen(cell(ii,jj).id).player != me()) {
                        if(citizen(cell(ii,jj).id).type == Warrior and not stronger_or_equal_weapon(citizen(id), citizen(cell(ii,jj).id))) safe = false;
                    }
                }
            } else safe = true;
        } else {
            if(is_round_night(round()) or (round() < num_rounds() and is_round_night(round()+5))) {
                for(int k = 0; k < 4; k++) {
                    ...
                    int jj = p.j + y[k];
                    if(pos_ok(Pos(ii,jj)) and cell(ii,jj).id != -1 and citizen(cell(ii,jj).id).player != me()) {
                        if(citizen(cell(ii,jj).id).type != Builder) safe = false;
                    }
                }
            } else safe = true;
    ...
    bool stronger_or_equal_weapon(Citizen c1, Citizen c2) {
        if((c1.weapon == Bazooka and c2.weapon == Bazooka) or (c1.weapon == Gun and c2.weapon == Gun) or (c1.weapon == Hammer and c2.weapon == Hammer)) return true;
    ...
    bool make_bunker(int id, Pos p) {
        if(cell(p).b_owner == me() and not safe_position(id,p)) {
            for(int k = 0; k < 4; k++) {
                int ii = p.i + x[k];
                int jj = p.j + y[k];
                Pos q = Pos(ii,jj);
        ...
        for(int k = 0; k < 4; k++) {
            int ii = p.i + x[k];
            int jj = p.j + y[k];
            if(pos_ok(Pos(ii,jj)) and cell(ii,jj).id == -1 and cell(ii,jj).type == Street and safe_position(id,Pos(ii,jj)) and cell(ii,jj).b_owner == -1) {
                Pos q = Pos(ii,jj);
                ...
                return true;
            } else if (pos_ok(Pos(ii,jj)) and cell(ii,jj).id == -1 and cell(ii,jj).type == Street and safe_position(id,Pos(ii,jj)) and cell(ii,jj).b_owner == me()) {
                Pos q = Pos(ii,jj);
                if(p.i + 1 == q.i) move(id,Down);
                else if(p.i - 1 == q.i) move(id,Up);
                else if(p.j + 1 == q.j) move(id,Right);
        ...
        while(!q.empty()) {
            ...
            and citizen(cell(t).id).type == Warrior and not shared_objective(t,dist[t.i][t.j])) {
                ...
                for(int k = 0; k < 4; k++) {
                    ...
                    int jj = t.j + y[k];
                    if(pos_ok(Pos(ii,jj)) and cell(Pos(ii,jj)).type != Building 
                    and dist[ii][jj] == -1 and safe_position(cell(p).id,Pos(ii,jj)) and (cell(ii,jj).id == -1
    ...
    void decide_objective() {
        ...
        while(itDec != movements.end()) {
            ...
            Pos p = c.pos;
            bool bunker = false;
            if(c.type ==  Builder and is_round_day(round()) and round() < num_rounds() and is_round_night(round()+3) and not safe_position(id,p) and int(barricades(me()).size()) < max_num_barricades()) {
                make_bunker(id,p);
            }
...
//al buscar enemics nit, no intentar passar per barricades de max health.
