  ...
  static Player* factory () {
    ...
    void initialize() {
        ...
        if(round()==0) {
            ...
            //cerr << endl;
            //cerr << "1";
            while(itIni != movements.end()) {
                //cerr << endl;
                //cerr << "2";
                if(not alive(itIni->first)) {/*cerr << endl;cerr << itIni->first << " is dead" << endl;*/movements.erase(itIni);}
                ++itIni;
            }
            vector<Pos> v;
            for(int i : warriors(me())) {
                //cerr << endl;
                //cerr << "4";
                if(resurected(i)) {
                    //cerr << endl;
                    //cerr << "5";
                    movements.insert(make_pair(i,make_pair(v,false)));
                } else {
                    //cerr << endl;
                    //cerr << "6";
                    itIni = movements.find(i);
                    itIni->second.second = false;
                    if(itIni->second.first.size()!=0)itIni->second.first.erase(itIni->second.first.begin());
                }
            }
            for(int j : builders(me())) {
                // cerr << endl;
                // cerr << "7";
                if(resurected(j)) {
                    // cerr << endl;
                    // cerr << "8";
                    movements.insert(make_pair(j,make_pair(v,false)));
                } else {
                    // cerr << endl;
                    // cerr << "9";
                    itIni = movements.find(j);
                    itIni->second.second = false;
                    if(itIni->second.first.size()!=0)itIni->second.first.erase(itIni->second.first.begin());
                }
            }
        }
        itIni = movements.begin();
        // cerr << endl;
        // cerr << endl;
        // cerr << "Warriors : ";
        // for(int i : warriors(me())) cerr << i << " ";
        // cerr << endl;
        // cerr << "Builders : ";
        // for(int j : builders(me())) cerr << j << " ";
        // cerr << endl;
        // cerr << "Movements : ";
        // while(itIni!=movements.end()) {
            // cerr << itIni->first << " ";
    ...
    bool shared_objective(Pos p) {
        ...
        return (shared and num_objectives >=int(movements.size())-1);
    }
    bool valid_objective(int id) {
        bool valid = false;
        it itVal= movements.find(id);
        if(itVal != movements.end() and itVal->second.first.size()!=0) {
            Pos p = itVal->second.first[itVal->second.first.size()-1];
            if(cell(p).bonus != NoBonus or cell(p).weapon != NoWeapon or cell(p).id != -1) valid = true;
            for(Pos q : itVal->second.first) if(cell(q).type == Building and cell(q).id != -1) valid = false;
        } else {
            movements.erase(itVal);
        }
        return valid;
    }
    ...
    bool findFood(Pos p, vector<Node>& parents) {
        ...
        while(!q.empty()) {
            ...
            q.pop();
            Pos t = aux.x;
            if(cell(t).bonus == Food and not shared_objective(t)) {
                p = aux.x;
                parents = {aux};
                return true;
            } else {
    ...
    bool findWeapon(Pos p, vector<Node>& parents) {
        ...
        while(!q.empty()) {
            ...
            and cit.weapon != Bazooka and not shared_objective(t)) {
                p = aux.x;
                parents = {aux};
                return true;
            } else if (cell(t).weapon == Bazooka and cit.weapon == Bazooka  and not shared_objective(t)){
                p = aux.x;
                parents = {aux};
                return true;
            } else {
    ...
    bool findMoney(Pos p, vector<Node>& parents) {
        ...
        while(!q.empty()) {
            ...
            q.pop();
            Pos t = aux.x;
            if(cell(t).bonus == Money and not shared_objective(t)) {
                p = aux.x;
                parents = {aux};
                return true;
            } else {
    ...
    bool findWarrior(Pos p, vector<Node>& parents) {
        ...
        while(!q.empty()) {
            ...
            and citizen(cell(t).id).type == Warrior  and not shared_objective(t)) {
                p = aux.x;
                parents = {aux};
                return true;
            } else {
    ...
    bool findBuilder(Pos p, vector<Node>& parents) {
        ...
        while(!q.empty()) {
            ...
            if(cell(t).id != -1 
            and citizen(cell(t).id).player != me() 
            and citizen(cell(t).id).type == Builder  and not shared_objective(t)) {
                p = aux.x;
                parents = {aux};
                return true;
            } else {
    ...
    void decide_objective() {
        ...
        while(itDec != movements.end()) {
            ...
            // cerr << "hola";
            Pos p = c.pos;
            if(itDec->second.first.size()!=0 and itDec->second.first.size() < 10 and valid_objective(id)) confirm_movement(id,p);
            else {
