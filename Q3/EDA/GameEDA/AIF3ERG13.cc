 ...
 */
#define PLAYER_NAME AIF3ERG13_1
...
struct PLAYER_NAME : public Player {
  ...
  static Player* factory () {
            ...
            for(int k = 0; k < 4; k++) {
                ...
                if(pos_ok(Pos(ii,jj)) and cell(ii,jj).id != -1 and citizen(cell(ii,jj).id).player != me()) {
                    if(citizen(id).type == Builder and citizen(cell(ii,jj).id).type != Builder) safe = false;
                        ...
                        if(citizen(id).type == Warrior and stronger_weapon(citizen(id),citizen(cell(ii,jj).id))) safe = false;
                    }
                }
            }
        } else safe = true;
    ...
    void run_away(int id, Pos p) {
        for(int k = 0; k < 4; k++) {
            int ii = p.i + x[k];
            int jj = p.j + y[k];
            if(pos_ok(Pos(ii,jj)) and safe_position(id, p)) {
                Pos q = Pos(ii,jj);
                if(p.i + 1 == q.i) move(id,Down);
                else if(p.i - 1 == q.i) move(id,Up);
                else if(p.j + 1 == q.j) move(id,Right);
    ...
    void decide_objective() {
        ...
        while(itDec != movements.end()) {
            ...
            Pos p = c.pos;
            bool bunker = false;
            if(c.type ==  Builder and ((is_round_day(round()) and round() < num_rounds() and is_round_night(round()+3)) or is_round_night(round())) and not safe_position(id,p)) {
                make_bunker(id,p);
            }
            else if(not decision_confirmed(id) and Healthcare(id)) confirm_movement(id,p);
            else if(c.type == Warrior and ArmYourself(id)) confirm_movement(id,p);
            else if(FarmingSimulator(id)) confirm_movement(id,p);
            //else if(not safe_position(id,p)) run_away(id, p);
            ++itDec;
        }
