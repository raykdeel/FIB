  ...
  static Player* factory () {
    ...
    void Healthcare() {
    	...
    	for (int id : w) {
            ...
            if(alive(c) and c.life <= warrior_ini_life()/2) {
                directionFood.clear();
                //cerr << endl;cerr << "Warrior " << c.id << " " << c.pos << endl;
                findFood(p,directionFood);
                //cerr << "Food: " << p;for(int i : directionFood) cerr << dirs[directionFood[i]];
                //cerr << endl;
                if(directionFood.size() != 0 and directionFood.size() < 6) {
                    //cerr<<"goingIn";
	    ...
	    for (int id : b) {
            ...
            if(alive(c) and c.life <=  builder_ini_life()/2) {
                directionFood.clear();
                //cerr << endl;cerr << "Builder " << c.id << " " << c.pos << endl;
                findFood(p,directionFood);
                //cerr << "Food: " << p;for(int i : directionFood) cerr << dirs[directionFood[i]];
                //cerr << endl;
                if(directionFood.size() != 0 and directionFood.size() < 6) {
    ...
    void ArmYourself() {
    	...
    	for (int id : w) {
            ...
            if(alive(c) and c.weapon <= Bazooka) {
                ...
                //cerr << "Weapon: " << p;for(int i : directionWeapon) cerr << dirs[directionWeapon[i]];
                //cerr << endl;
                if(directionWeapon.size()!=0) {
                    if((cell(p).weapon > c.weapon and directionWeapon.size() < 10) or (directionWeapon.size()<5)) {cerr<<"goingIn";move(id, dirs[directionWeapon[0]]);}
                }
            }
        }
    	...
    	for (int id : w) {
            ...
            if(alive(c)) {
                if(is_round_night(round()) or (round() < num_rounds()  and is_round_night(round()+1))) {
                    ...
                    //cerr << "Enemy: " << p;for(int i : directionEnemy) cerr << dirs[directionEnemy[i]];
                    //cerr << endl;                
                    ...
                    //cerr << "Money: " << p;for(int i : directionMoney) cerr << dirs[directionMoney[i]];cerr << endl;
                    if(directionMoney.size() != 0) {
                        //cerr<<"goingIn";
   ...
   void Attack(Pos& p,vector<int>& direction) {
        ...
        while(!Q.empty() and not found) {
            ...
            (citizen(cell(q.i,q.j).id).weapon < citizen(cell(p).id).weapon or citizen(cell(p).id).type == Builder)) {
                found = true;
                p.i = q.i; p.j = q.j;
                direction = t;              
            }
                ...
                while(k < 4 and not found) { 
                    ...
                    and not seen[u][v]) {
                        t.push_back(k);
                        if(cell(u,v).id != -1 and citizen(cell(u,v).id).player != me() and //u,v
            (citizen(cell(q.i,q.j).id).weapon < citizen(cell(p).id).weapon or citizen(cell(p).id).type == Builder)) {
                            found = true;
                            p.i = u; p.j = v;
                            direction = t;
